...
public class ReportTwoEyed {
	...
	public static void displayGraph(double sm_box,double bg_box,double lambda,double scale,int bits,int code )
	{
		...
		BufferedImage eye ;
		EyeDataType ed;
		boolean[] skip = new boolean[listOfFiles.length];
		...
		TwoEyedPerson[] tep = new TwoEyedPerson[listOfFiles.length/2];
		int countTEP =0;
		for (int i =0;i<count;i++)
			if (!skip[i] && is_odd(names[i])) 
			{
				for (int j =0;j<count;j++)
					if (!skip[j] && (convert_to_int(names[j])==convert_to_int(names[i])+1) )
					{
						tep[countTEP] = new TwoEyedPerson(convert_to_int(names[i]),bc[i],bc[j]);
						countTEP++;
System.out.println(names[i]+"  "+ names[j]);
		...
		for (int i=0;i<countTEP;i++)
		{
			for (int j=i+1;j<countTEP;j++)
			{			
				...
				//{
					if (hamm>highest) highest=hamm;
					if (hamm<lowest) lowest=hamm;
					if (TwoEyedPerson.samePerson(tep[i], tep[j]))
					{
						if (hamm>weakest_match) weakest_match = hamm;
		...
		if (overlap < 1700){
			...
			Graphics g  = biGraph.createGraphics();
		 	...
		 	isl.saveImage(biGraph,"TwoEyeHamming_Graph"+overlap+"_"+code+".gif");
	...
	private static boolean is_odd(String str)
	{
		if (convert_to_int(str)%2==1) return true;
		return false;
	}
	private static boolean is_even(String str)
	{
